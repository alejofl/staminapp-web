...
export default {
  ...
  methods: {
    ...
    setProfileInfo(){
      console.log("Actualizo los valores para el perfil en setProfileInfo")
      ...
      this.unsavedBirthDate = this.currentUser.birthdate
      this.unsavedMail = this.currentUser.mail
      //this.unsavedHeight = this.currentUser.height[this.$currentUser.length - 1]
      //   this.unsavedWeight = this.$currentUser.weight[this.$currentUser.length - 1]
      this.unsavedBase64Data = this.currentUser.base64Data
    }
  },
  beforeDestroy() {
    this.currentUser.name = this.unsavedName
    this.currentUser.gender = this.unsavedGender
    this.currentUser.birthdate = this.unsavedBirthDate
    //this.unsavedHeight = this.currentUser.height[this.$currentUser.length - 1]
    //   this.unsavedWeight = this.$currentUser.weight[this.$currentUser.length - 1]
